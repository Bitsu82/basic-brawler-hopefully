...
from basic_brawler import State
...
def draw_healthbar(surface,x,y,current_health,max_health):
    bar_width = 200
    bar_height = 20
    fill = int((current_health / max_health) * bar_width)
...
while True:   
    ...
    #guy1 controls
    if not guy1.is_dead():
        if keys[pygame.K_a] and guy1.state == State.IDLE:
            guy1.rect.x -= 3
            guy1.facing = 'left'
        if keys[pygame.K_d] and guy1.state == State.IDLE:
            guy1.rect.x += 3
            guy1.facing = 'right'
        #guy 2 controls
    if not guy2.is_dead():
        if keys[pygame.K_LEFT] and guy2.state == State.IDLE:
            guy2.rect.x -= 3
            guy2.facing = 'left'
        if keys[pygame.K_RIGHT] and guy2.state == State.IDLE:
            guy2.rect.x += 3
            guy2.facing = 'right'
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if not guy2.is_dead():
                if event.key  ==  pygame.K_UP and guy2.state == State.IDLE:
                    guy2.jump()
                if event.key == pygame.K_m and guy1.state == State.IDLE:
                    guy2.punch(guy1)
    ...
    draw_healthbar(screen, 50, 30, guy1.health, guy1.max_health)
    draw_healthbar(screen, 950, 30, guy2.health, guy2.max_health)

    if guy1.is_dead():
        text = font.render("P2 Wins!", True, (255, 255, 255))
        screen.blit(text, text.get_rect(center=(WIDTH // 2, HIEGHT // 2)))

    if guy2.is_dead():
        text = font.render("P1 Wins!", True, (255, 255, 255))
        screen.blit(text, text.get_rect(center=(WIDTH // 2, HIEGHT // 2)))
    if guy1.punch_hitbox:

        pygame.draw.rect(screen, (255, 0, 0), guy1.punch_hitbox, 2)
    if guy2.punch_hitbox:
        pygame.draw.rect(screen, (255, 255, 0), guy2.punch_hitbox, 2)
    if guy1.shield_hitbox:
        pygame.draw.rect(screen, (255, 0, 0), guy1.shield_hitbox,2)
    if guy2.shield_hitbox:
        pygame.draw.rect(screen, (255, 255, 0), guy2.shield_hitbox, 2)
